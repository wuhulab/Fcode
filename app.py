...
def workspace_status():
    ...
    if workspace_root:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"workspace": None, "name": None, "configured": False},
            }
        )
        ...
        return jsonify(
            {
                "success": True,
                "message": "工作目录设置成功",
                "data": {"workspace": normalized, "name": os.path.basename(normalized)},
            }
        )
        ...
        try:
            ...
            for item in items:
                ...
                if os.path.isdir(full_path):
                    ...
                    directories.append(
                        {"name": item, "path": full_path, "readable": readable}
                    )
...
def get_files():
    try:
        ...
        if not workspace_root:
            return jsonify(
                {"error": "工作目录未设置，请先选择项目文件夹", "needWorkspace": True}
            ), 400
...
def read_file():
    try:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"path": file_path, "content": content, "size": file_size},
            }
        )
...
def create_file():
    try:
        ...
        return jsonify(
            {"success": True, "message": f"{file_type} 创建成功", "path": new_path}
        )
    ...
    try:
        ...
        if ext in [".html", ".htm"]:
            try:
                ...
                return jsonify(
                    {"success": True, "message": "已在浏览器中打开", "output": ""}
                )
            except Exception as e:
        ...
        try:
            ...
            return jsonify(
                {"error": f"命令未找到: {command_base}，请确保已安装相关环境"}
            ), 404
        except PermissionError:
...
def terminal_execute():
    ...
    try:
        ...
        if len(command) > 1000:
            return jsonify(
                {"error": "命令过长，最大允许1000字符", "success": False}
            ), 400
        ...
        while _pty_processes.get(sid, {}).get("alive", False):
            try:
                ...
                if hasattr(pty_process, "isalive") and not pty_process.isalive():
                    break
                # winpty read() 使用 blocking 参数
                # 尝试非阻塞读取，如果无数据则短暂休眠
                data = pty_process.read(blocking=False)
                if data:
                ...
                if (
                    ...
                    or "not ready" in error_str.lower()
                ):
                    break
                # 短暂休眠后重试
                socketio.sleep(0.01)
...
def handle_terminal_create(data):
    ...
    try:
        ...
        with _pty_lock:
            _pty_processes[sid] = {"process": pty_process, "alive": True}

        # 启动后台读取线程
        read_thread = threading.Thread(
            target=_read_pty_output, args=(sid, pty_process), daemon=True
        )
        read_thread.start()
    ...
    try:
        ...
        if not os.path.exists(git_dir):
            return jsonify(
                {
                    "success": True,
                    "data": {"isRepo": False, "message": "当前目录不是Git仓库"},
                }
            )
...
def git_log():
    try:
        ...
        return jsonify({"success": True, "data": {"commits": commits}})
...
def git_init():
    try:
        ...
        return jsonify(
            {"success": True, "message": "Git仓库初始化成功", "output": result.stdout}
        )
...
def git_add():
    try:
        ...
        return jsonify(
            {"success": True, "message": "文件已添加到暂存区", "output": result.stdout}
        )
...
def git_commit():
    try:
        ...
        return jsonify(
            {"success": True, "message": "提交成功", "output": result.stdout}
        )
...
def git_push():
    try:
        ...
        return jsonify(
            {"success": True, "message": "推送成功", "output": result.stdout}
        )
...
def git_pull():
    try:
        ...
        return jsonify(
            {"success": True, "message": "拉取成功", "output": result.stdout}
        )
...
def git_branch():
    try:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"branches": branches, "currentBranch": current_branch},
            }
        )
...
def git_diff():
    try:
        ...
        return jsonify({"success": True, "data": {"diff": result.stdout}})
